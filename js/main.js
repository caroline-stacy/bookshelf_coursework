  ...
  return `
    <div class="book-card" data-id="${book.id}">
      ...
      <div class="card-footer">
        <select class="card-status-select" data-id="${book.id}" aria-label="Статус чтения">
          <option value="want"    ${book.status === 'want'    ? 'selected' : ''}> Хочу читать</option>
...
function attachCardEvents() {
  ...
  document.querySelectorAll('.card-status-select').forEach(function attachStatusSelect(sel) {
    sel.addEventListener('change', function onStatusChange() {
      changeBookStatus(Number(sel.dataset.id), sel.value);
    });
  });
...
/**
 * Сменить статус книги через выпадающее меню
 * @param {number} id
 * @param {string} newStatus — 'want' | 'reading' | 'read'
 */
